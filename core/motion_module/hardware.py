...
HARDWARE = {
    ...
    #
    # On the reference hardware every driver's output A uses inverted polarity,
    # so channels 1, 3, 5 and 7 are inverted here and the output B channels
    # are not. A robot project can flip `inverted` for its own mounted motors;
    # never move a pin or compensate for one motor in the drive math.
    # ------------------------------------------------------------------
    "motors": {
