...
def _reference(value: str) -> int | str:
    """Bench commands accept a hardware.py name or a plain channel number."""

    text = str(value).strip()
    return int(text) if text.isdigit() else text


def test_motor(channel: int | str, power: float, seconds: float, yes: bool) -> int:
    channel = _reference(channel)
    ...
    with MotionModule() as controller:
        ...
        print(f"Pulsing {motor.name} (channel {motor.channel}) at {power:+.0%} for {seconds:.2f} seconds")
        motor.set(power)
...
def test_servo(board: int, channel: int | str, angle: float, yes: bool) -> int:
    channel = _reference(channel)
    _require_bench_confirmation(yes)
    if not 0 <= angle <= 180:
    ...
    with MotionModule() as controller:
        ...
        print(f"Moving {servo.name} (board {servo.board}, channel {servo.channel}) to {angle:.1f} degrees")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    subparsers.add_parser("pinout", help="Print the active names, physical pins, and GPIO map")
    motor_parser = subparsers.add_parser("test-motor", help="Pulse one raised-wheel motor")
    motor_parser.add_argument("channel", help="Motor name from hardware.py, or channel 1-8")
    ...
    servo_parser.add_argument("channel", help="Servo name from hardware.py, or channel 0-15")
