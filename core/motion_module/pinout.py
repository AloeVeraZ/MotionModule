...
def header_rows(config, *, imu_guide: bool = False) -> list[dict]:
    ...
    if imu_guide:
        for physical, label in ((6, "GND signal ground"), (20, "AD0 secondary ground / address select (0x68)"), (11, "SDA / GPIO17"),
            ...
            roles[physical] = (f"Optional MPU6500 IMU {label}", "reserved")
            details[physical] = (
        ...
        rows.append(
            {
                ...
                "connection": role,
                "detail": details.get(physical, "Not used by the reference robot. Additional sensors use USB or the optional Arduino GPIO expansion."),
            }
        )
    return rows
